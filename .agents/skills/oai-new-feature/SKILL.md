...
2. **Bump or create the relevant skill**: if your feature is large enough to need its own context, write a sibling skill at `.agents/skills/oai-<feature>/SKILL.md` (model on `oai-chat` / `oai-img`) and add it to the activation matrix in `CLAUDE.md`.
