...
ResponseLike = requests.Response
...
def report_result(transport: ReportClient, report: TaskResultReport) -> bool:
    ...
    try:
        logger.info("Reporting result for task id=%s cap=%s", q.id, q.cap)
        resp = _retry_post(
        ...
        if resp.content:
            try:
                logger.debug("%s", resp.content.decode("utf-8", errors="ignore"))
            except Exception:
                pass
        logger.info("Task result reported. Status Code: %s", resp.status_code)
        return True
    except TaskCancelled:
        ...
        logger.error("Failed to report task result after retries: %s", e)
        return False
