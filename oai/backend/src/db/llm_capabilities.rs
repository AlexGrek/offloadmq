...
use crate::{
    db::entities::llm_capabilities::{ActiveModel, Column, Entity, Model},
    error::AppError,
...
/// All stored models for the picker: online first, then offline by most recent availability.
...
mod tests {
    ...
    #[test]
    fn stale_cutoff_is_three_days() {
        assert_eq!(STALE_AFTER.num_days(), 3);
    }

    #[test]
    fn sort_order_online_before_offline() {
        use crate::db::entities::llm_capabilities::Model as LlmCapRow;

        let online = HashSet::from(["llm.a".to_string()]);
        ...
        assert_eq!(rows[0].base, "llm.a");
        assert_eq!(rows[1].base, "llm.b");
    }
}
