//! WebSocket transport for Movie Studio: live job updates while the director /
//! scene-director / video state machine reconciles.
...
use crate::ws::events::{MovieClientCommand, ServerEvent};

const PING_INTERVAL: Duration = Duration::from_secs(30);
const IDLE_TIMEOUT: Duration = Duration::from_secs(120);

struct MovieConnectionScope {
    open: AtomicBool,
    watch: Mutex<Option<AbortHandle>>,
}

impl MovieConnectionScope {
    fn new() -> Self {
        Self {
...
async fn run_connection(socket: WebSocket, state: Arc<AppState>, user_id: i64) {
    let scope = Arc::new(MovieConnectionScope::new());
...
async fn reader_loop(
    ...
    state: Arc<AppState>,
    user_id: i64,
    scope: Arc<MovieConnectionScope>,
) {
    ...
    loop {
        ...
        let frame = tokio::select! {
            ...
            _ = &mut timeout => {
                tracing::debug!("movie ws idle timeout user={user_id}");
                return;
            }
        };
...
async fn handle_text(
    ...
    let cmd = match serde_json::from_str::<MovieClientCommand>(text) {
        Ok(c) => c,
        Err(_) => return,
    };
    match cmd {
        MovieClientCommand::Ping => {
            let _ = tx.send(ServerEvent::Pong);
        }
        MovieClientCommand::ListCapabilities { req_id } => {
            movie::list_capabilities_ws(req_id, tx, state, user_id).await;
        }
        MovieClientCommand::WatchJob { req_id, job_id } => {
            let tx = tx.clone();
            ...
            let task = tokio::spawn(async move {
                ...
                movie::watch_job_ws(req_id, job_id, &tx, &state, user_id).await;
            });
