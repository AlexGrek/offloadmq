...
import type { LlmCompareJob } from '../../api/llmCompare'
import { capabilityBaseLabel } from '../../lib/modelAvailability'

const TERMINAL = new Set(['completed', 'failed', 'canceled'])
export const LLM_COMPARE_NEW_PANEL = 'new' as const

type LlmCompareHistorySidebarProps = {
  jobs: LlmCompareJob[]
  activePanel: string
  loading?: boolean
  onSelectNew: () => void
...
export function LlmCompareHistorySidebar({
  jobs,
  activePanel,
  loading,
  onSelectNew,
  onSelectJob,
}: LlmCompareHistorySidebarProps) {
  const isNewActive = activePanel === LLM_COMPARE_NEW_PANEL
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
      <button
        type="button"
        onClick={onSelectNew}
        data-testid="llm-compare-new-panel-btn"
        className={cn(
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-1 py-1">
        {loading ? (
          ...
          <p className="px-3 py-4 text-center text-xs text-muted-foreground">No comparisons yet</p>
        ) : (
          <ul className="space-y-1">
            {jobs.map((job, i) => {
              const active = activePanel === job.job_id
              const inProgress = statusLabel(job.status)
              const models = job.slots.map(s => capabilityBaseLabel(s.model)).join(' · ')
              return (
                <motion.li
                  key={job.job_id}
                ...
                >
                  <button
                    ...
                    data-testid={`llm-compare-item-${job.job_id}`}
                    className={cn(
