...
export const MOVIE_NEW_PANEL = 'new' as const

type MovieHistorySidebarProps = {
  jobs: MovieJobView[]
  activePanel: string
  loading?: boolean
  onSelectNew: () => void
...
function jobTitle(idea: string, limit = 72): string {
...
export function MovieHistorySidebar({
  jobs,
  activePanel,
  loading,
  onSelectNew,
  onSelectJob,
}: MovieHistorySidebarProps) {
  const isNewActive = activePanel === MOVIE_NEW_PANEL
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
      <button
        type="button"
        onClick={onSelectNew}
        data-testid="movie-new-panel-btn"
        className={cn(
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-1 py-1">
        {loading ? (
          ...
          <p className="px-3 py-4 text-center text-xs text-muted-foreground">No movies yet</p>
        ) : (
              ...
              return (
                ...
                >
                  <button
                    ...
                    data-testid={`movie-item-${job.job_id}`}
                    className={cn(
                  ...
                  >
                    <div className="relative z-10 flex min-h-17 flex-col justify-center gap-0.5 px-3 py-2">
                      ...
                      <span className="truncate pl-5 font-mono text-[10px] text-muted-foreground/80">
                        {job.scene_count} scenes · {job.phase}
                      </span>
                      <span className="pl-5">
                        {inProgress ? (
