...
export const TTS_NEW_PANEL = 'new' as const

type TtsHistorySidebarProps = {
  jobs: TtsJob[]
  /** `'new'` or a job id */
  activePanel: string
  loading?: boolean
...
function jobTitle(text: string, limit = 72): string {
...
export function TtsHistorySidebar({
  jobs,
  activePanel,
  loading,
  onSelectNew,
  onSelectJob,
}: TtsHistorySidebarProps) {
  const isNewActive = activePanel === TTS_NEW_PANEL
    ...
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain py-1 px-1">
        {loading ? (
          ...
          <p className="px-3 py-4 text-center text-xs text-muted-foreground">No speech yet</p>
        ) : (
              ...
              return (
                <li key={job.job_id}>
                  <button
                    ...
                    data-testid={`tts-item-${job.job_id}`}
                    className={cn(
                      'group/tts relative w-full overflow-hidden rounded-lg text-left transition-colors',
                  ...
                  >
                    <div className="relative z-10 flex min-h-17 flex-col justify-center gap-0.5 px-3 py-2">
                      <div className="flex items-start gap-1.5">
                        <Volume2 className="mt-0.5 size-3.5 shrink-0 text-muted-foreground" />
