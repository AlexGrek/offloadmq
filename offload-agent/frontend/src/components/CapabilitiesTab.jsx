...
function SlavemodeSection({ state, loadState, run }) {
  ...
  if (allSlavemodeCaps.length === 0) return null

  return (
    <div className="mt-6 border border-amber-800/40 rounded-lg p-4 bg-amber-950/20">
      <h2 className="text-[0.72rem] font-semibold text-amber-500/80 uppercase tracking-wider mb-1">
        Slavemode Permissions
      </h2>
      <p className="text-xs text-slate-500 mb-3">
        Allow the server to trigger control operations on this agent. Disabled by default.
      </p>
...
export function CapabilitiesTab({ state, loadState, run }) {
  ...
  return (
    <div className="bg-slate-800 rounded-lg p-5">
      ...
      <SlavemodeSection state={state} loadState={loadState} run={run} />
    </div>
  )
}
