...
def _resolve_workflow_graph_path(workflow_name: str, task_type: str) -> Path:
    """Return the resolved path to `{task_type}.json` under a workflow folder."""
    wf = workflow_name.strip()
    tt = task_type.strip()
    if not wf or not _WF_SAFE_RE.match(wf):
        raise ValueError("invalid workflow_name")
    if not tt or not _WF_SAFE_RE.match(tt):
        raise ValueError("invalid task_type")
    root = _workflows_dir().resolve()
    base = (_workflows_dir() / wf).resolve()
...
def _validate_comfy_api_workflow(graph: Any) -> None:
    """Basic structural checks for ComfyUI API-format workflow JSON."""
...
async def save_workflow_param_map(request: Request):
    ...
    _log(f"[imggen] Saved param map: {params_path}")
    _start_scan()
    return JSONResponse({"ok": True})
...
async def autodetect_workflow_param_map(request: Request):
    ...
    guessed = _guess_params(graph, tt)
    stubs = _default_params(tt)
    merged = {**stubs, **guessed}
    params_path = graph_path.with_suffix(".params.json")
    with open(params_path, "w", encoding="utf-8") as f:
        json_module.dump(merged, f, indent=2)
    _log(f"[imggen] Overwrote param map with auto-detect: {params_path}")
    _start_scan()
