...
class TaskResultStatus:
    ...
    def to_json(self) -> dict:
        """
        Serializes a TaskResultStatus dataclass to a JSON string.
        """
        if self.status == "success":
            return ({"success": self.data.total_seconds()})
        elif self.status == "failure":
            error_message, duration = self.data
            return ({"failure": [error_message, duration.total_seconds()]})
        elif self.status == "notExecuted":
...
def execute_debug_echo(
    task_id: TaskId, capability: str, payload: dict, server_url: str, headers
):
    """
    ...
    try:
        ...
        print(
            f"Task result for {task_id} reported successfully. Status Code: {response.status_code}"
        )
        return True
...
def execute_shell_bash(
    task_id: TaskId, capability: str, payload: dict, server_url: str, headers: dict
):
    """
    ...
    if not command:
        ...
        report = TaskResultReport(
            task_id=task_id, status=TaskResultStatus(status="failure", data=(str(error_output), timedelta(seconds=12.5))), output=error_output, capability=capability
        )
        report_url = f"{server_url}/private/agent/task/resolve/{report.task_id.cap}/{report.task_id.id}"
        try:
            requests.post(
                report_url, json=report.to_json(), headers=headers
            ).raise_for_status()
        except requests.exceptions.RequestException as e:
    ...
    try:
        ...
        report_output = {"stdout": result.stdout, "stderr": result.stderr}
        report = TaskResultReport(
            task_id=task_id,
            status=TaskResultStatus(status="success", data=timedelta(seconds=12.5)),
        ...
        report = TaskResultReport(
            task_id=task_id,
            status=TaskResultStatus(status="failure", data=(e.stderr, timedelta(seconds=5.0))),
            output=report_output,
            capability=capability,
        )
...
def execute_llm_query(
    ...
    try:
        ...
        if not prompt:
            ...
            report = TaskResultReport(
                task_id=task_id,
                status=TaskResultStatus(status="failure", data=(str(error_output), timedelta(seconds=5.0))),
                output=error_output,
                capability=capability,
            )
            report_url = f"{server_url}/private/agent/task/resolve/{report.task_id.cap}/{report.task_id.id}"
        ...
        print(
            f"Executing LLM query for task {task_id} with model '{model_name}' via API."
        )
        ...
        report_output = {
            ...
            "response_text": (
                e.response.text if e.response else "No response from server"
            ),
        }
        report = TaskResultReport(
            task_id=task_id,
            status=TaskResultStatus(status="failure", data=(str(report_output), timedelta(seconds=5.0))),
            output=report_output,
            capability=capability,
        )
    except Exception as e:
        ...
        while True:
            ...
            try:
                ...
                if task_info and task_info.get("id"):
                    ...
                    take_url = (
                        f"{server_url}/private/agent/take/{task_cap}/{task_id_str}"
                    )
                    take_response = requests.post(take_url, headers=headers)
                    take_response.raise_for_status()

                    task = take_response.json()
                    task_id = parse_id(task.get("id"))
                    capability = task_id.cap
                    payload = task.get("data").get("payload")

                    print(
                        f"Received new task: {task_id.to_json()} with capability '{capability}'"
                    )

                    # Route to the correct executor
                    if capability.startswith("LLM::"):
                        execute_llm_query(
                            task_id, capability, payload, server_url, headers
                        )
                    else:
                        ...
                        if executor:
                            ...
                            requests.post(
                                report_url, json=error_report.to_json(), headers=headers
                            )
...
def get_ollama_models() -> List[str]:
    ...
    try:
        ...
        print(
            f"Warning: An unexpected error occurred while detecting Ollama models: {e}"
        )
...
def authenticate_agent(server: str, agent_id: str, key: str) -> Dict[str, Any]:
    ...
    auth_data = {"agentId": agent_id, "key": key}
...
def start_ollama_server():
    ...
    try:
        ...
        print(
            "Error: 'ollama' command not found. Please ensure Ollama is installed and in your system's PATH."
        )
        return False
    except Exception as e:
...
def main():
    ...
    register_parser = subparsers.add_parser(
        "register", help="Register a new agent with the server"
    )
    register_parser.add_argument(
        "--server", help="Server URL (required if not in config)"
    )
    register_parser.add_argument("--key", help="API key (required if not in config)")
    register_parser.add_argument(
        "--tier", type=int, default=5, help="Performance tier (0-255, default: 5)"
    )
    register_parser.add_argument(
        "--caps",
        nargs="*",
        default=["debug::echo", "shell::bash"],
        help="Agent capabilities (default: ['debug::echo', 'shell::bash'])",
    )
    register_parser.add_argument(
        "--capacity", type=int, default=1, help="Concurrent task capacity (default: 1)"
    )
    ...
    serve_parser = subparsers.add_parser(
        "serve", help="Periodically poll for and execute tasks"
    )
    ...
    if args.action == "sysinfo":
        ...
        registration_response = register_agent(
            server, combined_capabilities, args.tier, args.capacity, api_key
        )
        ...
        auth_response = authenticate_agent(
            server, registration_response["agentId"], registration_response["key"]
        )
        ...
        if not all([agent_id, key]):
            print(
                "Error: Agent not registered or config file is incomplete. Please run 'register' first."
            )
            sys.exit(1)
